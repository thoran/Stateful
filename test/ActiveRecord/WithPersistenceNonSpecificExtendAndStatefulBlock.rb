...
describe Stateful::ActiveRecord do
  ...
  context "initial_state" do
    ...
    it "must have an initial state consistent with what is given" do
      machine.initial_state.must_equal ActiveRecordMachine1.stateful_states.initial_state
    end
    ...
    it "must know what it's next state is given an event name" do
      machine.next_state(:an_event).must_equal ActiveRecordMachine1.stateful_states.find(:next_state)
      machine.next_state(:another_event).must_equal ActiveRecordMachine1.stateful_states.find(:final_state)
    end
