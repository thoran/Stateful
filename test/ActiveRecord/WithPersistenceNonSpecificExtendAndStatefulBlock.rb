...
unless ActiveRecord::Base.connection.tables.include?('active_record_machine1s')
  CreateTableMachines.new.change
end
if ActiveRecord::Base.connection.tables.include?('active_record_machine1s')
  ActiveRecordMachine1.delete_all
end
...
describe Stateful::ActiveRecord do
  ...
  context "initial_state" do
    ...
    it "must have an initial state consistent with what is given" do
      _(machine.initial_state).must_equal ActiveRecordMachine1.stateful_states.initial_state
    end
    ...
    it "must know what it's next state is given an event name" do
      _(machine.next_state(:an_event)).must_equal ActiveRecordMachine1.stateful_states.find(:next_state)
      _(machine.next_state(:another_event)).must_equal ActiveRecordMachine1.stateful_states.find(:final_state)
    end
  ...
  context "final_state" do
    ...
    it "must have a final state consistent with what is given" do
      _(machine.final_state).must_equal ActiveRecordMachine1.stateful_states.final_state
    end
